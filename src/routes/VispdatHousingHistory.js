...
class VispdatHistoryHousing extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      questions: VispdatHousingHistory.get('questions').map((question) => {
  ...
  _renderPrefaceText() {
    if (VispdatHousingHistory.prefaceText) {
      return (<Text>{VispdatHousingHistory.get('prefaceText')}</Text>);
    }
    return null;
  }
  ...
  render() {
    ...
    return (
      <ScrollView>
        ...
        />
        <Header
          text={VispdatHousingHistory.get('sectionTitle')}
        />
        <Questions
...
export default VispdatHistoryHousing;
